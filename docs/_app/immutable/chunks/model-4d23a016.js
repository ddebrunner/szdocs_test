...
`;class $T{constructor(s){tl(this,"mapping");this.mapping=s}getFor(s){return this.mapping.get(s.name)}}var wr;(function(t){t.UNION="union",t.ARGUMENT="argument",t.FIELD="field"})(wr||(wr={}));function GT(t){const s=new Map;return VT(t,i=>{Ke(i)&&QT(i,o=>{const u=ha(o.type);ua(s,u).references.push({kind:wr.FIELD,parent:i,by:o}),jT(o,m=>{const T=ha(m.type);ua(s,T).references.push({kind:wr.ARGUMENT,field:o,type:i,by:m})})}),Dn(i)&&i.getTypes().forEach(o=>{ua(s,o).references.push({kind:wr.UNION,by:i})})}),new $T(s)}function VT(t,s){Object.entries(t.getTypeMap()).forEach(([,i])=>{i.name.startsWith("__")||s(i)})}function QT(t,s){Object.entries(t.getFields()).forEach(([,i])=>{s(i)})}function jT(t,s){Object.entries(t.args).forEach(([,i])=>{s(i)})}function ua(t,s){let i=t.get(s.name);return i||(i={references:[]},t.set(s.name,i)),i}function ha(t){return Qe(t)||Dt(t)?ha(t.ofType):t}const We=r0(),ma=ev(jy.DIRECTIVES,[]);var yl;const WT=Ui((yl=We.getQueryType())==null?void 0:yl.getFields());var vl;const YT=Ui((vl=We.getMutationType())==null?void 0:vl.getFields());var Tl;const JT=Ui((Tl=We.getSubscriptionType())==null?void 0:Tl.getFields()),Na=vn.keyBy(rc(),t=>t.name.toLocaleLowerCase()),HT=vn.mapValues(Na,t=>{const s=ma.find(i=>(i==null?void 0:i.name)===t.name||(i==null?void 0:i.name)==="*");return s?s.args.some(i=>i==="*")?t.args:s.args.map(i=>t.args.find(o=>o.name===i)).filter(i=>!!i):[]}),zT=Ui(We.getTypeMap()),XT=GT(We);function Ui(t){return vn.mapKeys(t||{},(s,i)=>i.toLocaleLowerCase())}function KT(){return vn.size(We.getTypeMap())<=10}function c0(){return[la("Queries",We.getQueryType()),la("Mutations",We.getMutationType()),la("Subscriptions",We.getSubscriptionType()),i0(),n0()].filter(t=>!!t)}function la(t,s){return e0(t,ZT(s))}function ZT(t){return vn.sortBy((t==null?void 0:t.getFields())||{},s=>s.name)}function e0(t,s){return s.length===0?null:{type:"menu",title:t,children:s.map(i=>({type:"page",title:i.name,section:t,deprecated:!!i.deprecationReason,href:ga.joinUrlPaths(ya,t.toLocaleLowerCase(),i.name)}))}}function n0(){return KT()?null:{type:"menu",title:"Types",children:vn.sortBy(vn.map(We.getTypeMap()),s=>s.name).filter(s=>!s.name.startsWith("__")).map(s=>({type:"page",title:s.name,section:"Types",href:ga.joinUrlPaths(ya,"types",s.name)}))}}function f0(){return!!We.getQueryType()}function p0(t){return WT[t.toLocaleLowerCase()]}function d0(){return!!We.getMutationType()}function h0(t){return YT[t.toLocaleLowerCase()]}function m0(){return!!We.getSubscriptionType()}function g0(t){return JT[t.toLocaleLowerCase()]}function y0(t){return zT[t.toLocaleLowerCase()]}function t0(t){return Na[t.toLocaleLowerCase()]}function v0(t){return t0(t.name)!==void 0}function T0(){return vn.size(Na)>0}function _0(t){return HT[t.name.toLocaleLowerCase()]||[]}function E0(t){if(t)return XT.getFor(t)}function I0(t){return vn.flatMap(t.getFields(),s=>({field:s,possibleDescriptions:tc(s,t)}))}function tc(t,s){return t?t.description?[{description:t.description,from:s}]:Ke(s)?s.getInterfaces().flatMap(i=>tc(i.getFields()[t.name],i)):[]:[]}function rc(){return ma.some(t=>(t==null?void 0:t.name)==="*")?We.getDirectives().filter(t=>!["include","skip","deprecated","specifiedBy"].includes(t.name)):ma.filter(t=>!!(t!=null&&t.name)).map(({name:t})=>t?We.getDirective(t):void 0).filter(t=>!!t)}function r0(){return gl.trim().length===0?MT(JSON.parse(JSON.stringify({__schema:{types:[]}}))):qT(gl)}function i0(){const t=rc();if(t.length!==0)return{type:"menu",title:"Directives",children:t.map(s=>({type:"page",title:s.name,href:ga.joinUrlPaths(ya,"directives",s.name),section:"Directives"}))}}export{p0 as A,m0 as B,g0 as C,I0 as D,y0 as E,E0 as F,v as K,wr as R,Ke as a,je as b,yt as c,Dn as d,Cn as e,KT as f,ev as g,u0 as h,Rn as i,c0 as j,_0 as k,vn as l,Ml as m,Dt as n,Qe as o,l0 as p,T0 as q,t0 as r,We as s,jy as t,jv as u,Ul as v,v0 as w,d0 as x,h0 as y,f0 as z};
